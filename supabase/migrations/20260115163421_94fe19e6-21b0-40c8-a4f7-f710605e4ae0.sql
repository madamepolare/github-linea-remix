...
-- Add auto-followup configuration to crm_pipeline_stages
ALTER TABLE public.crm_pipeline_stages
ADD COLUMN IF NOT EXISTS auto_followup_enabled BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS auto_followup_days INTEGER DEFAULT 3,
ADD COLUMN IF NOT EXISTS auto_followup_action_title TEXT DEFAULT 'Relancer le contact';

-- Add tracking columns to contact_pipeline_entries
...
ADD COLUMN IF NOT EXISTS unread_replies_count INTEGER DEFAULT 0;

-- Create index for followup queries
CREATE INDEX IF NOT EXISTS idx_contact_pipeline_entries_awaiting 
ON public.contact_pipeline_entries(awaiting_response) 
WHERE awaiting_response = true;

-- Function to update unread_replies_count when inbound email arrives
...
-- Create trigger for inbound email replies
