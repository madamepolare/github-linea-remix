...
export function EntityDocumentsList({ entityType, entityId }: EntityDocumentsListProps) {
  const { navigate } = useWorkspaceNavigation();
  const { documents, isLoading } = useAgencyDocuments();
  const [search, setSearch] = useState("");

  const entityDocuments = documents.filter((d) => 
    entityType === "company" ? d.company_id === entityId : false
  );
        ...
        {filteredDocuments.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-muted-foreground text-sm max-w-md mb-4">
              Créez votre premier document lié à {entityType === "company" ? "cette entreprise" : "cette opportunité"}.
            </p>
