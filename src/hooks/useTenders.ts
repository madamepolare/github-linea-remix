...
export function useTenders() {
  ...
  const stats = {
    ...
    gagnes: tenders.filter(t => t.pipeline_status === 'gagnes').length,
    perdus: tenders.filter(t => t.pipeline_status === 'perdus').length,
    archives: tenders.filter(t => t.pipeline_status === 'archives').length,
    tauxReussite: (() => {
      const totalResults = tenders.filter(t => t.pipeline_status === 'gagnes' || t.pipeline_status === 'perdus').length;
      if (totalResults === 0) return 0;
      return Math.round((tenders.filter(t => t.pipeline_status === 'gagnes').length / totalResults) * 100);
    })(),
  };
