  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      const { data: parent, error: parentError } = await supabase
        .from("projects")
        .select("crm_company_id, project_type, color, name")
      ...
      // If supplementary work, create a draft quote linked to this sub-project
      if (data.billing_type === "supplementary") {
        const { data: draftQuote, error: quoteError } = await supabase
          .from("commercial_documents")
          .insert({
            workspace_id: activeWorkspace.id,
            created_by: userData.user.id,
            document_type: "quote",
            title: `Devis - ${data.name}`,
            description: data.description || `Travaux supplémentaires : ${data.name}`,
            status: "draft",
            project_id: newProject.id,
            client_company_id: parent.crm_company_id,
            project_type: parent.project_type || "other",
            fee_mode: "fixed",
        ...
        if (quoteError) {
          console.error("Error creating draft quote:", quoteError);
          // Don't fail the whole operation, just log the error
        } else {
          // Update the sub-project with the linked quote ID
          await supabase
            .from("projects")
            .update({ linked_quote_id: draftQuote.id })
            .eq("id", newProject.id);
        }
      }
    ...
    onSuccess: (_, variables) => {
      ...
      toast.success(variables.billing_type === "supplementary" 
        ? "Demande créée avec brouillon de devis" 
